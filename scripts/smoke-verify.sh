...
rm -f "${LOG_FILE}"
set +e
timeout "${TIMEOUT_SECONDS}s" env ELECTRON_ENABLE_LOGGING=1 "${APP_CMD}" --disable-gpu --disable-features=Vulkan >"${LOG_FILE}" 2>&1
status=$?
set -e
...
if grep -Eiq 'app server has not been initiali[sz]ed|CODEX_CLI_PATH is not set|could not find the Codex CLI|Cannot find module|ENOENT|EACCES|Trace/breakpoint trap|Segmentation fault' "${LOG_FILE}"; then
